...
from utils.org import (
    ...
    count_org_members, PLAN_TIERS, estimate_monthly_price,
)
...
total_mrr = sum(estimate_monthly_price(o["plan_tier"], o["max_users"]) for o in active_orgs)
...
with tab_new:
    ...
    with col1:
        plan_tier = st.selectbox(
            ...
            format_func=lambda k: f"{PLAN_TIERS[k]['name']} — {PLAN_TIERS[k]['label']} — {PLAN_TIERS[k]['price_per_user']}€/user/mois",
        )
    with col2:
    ...
    if st.button("🚀 Creer l'organisation et le compte admin", type="primary", use_container_width=True):
        ...
        if errors:
            ...
            ok, org_id_or_err = create_organization(org_name, plan_tier, max_users, contact_email, notes)
            if not ok:
...
with tab_list:
    if not orgs:
        ...
        for org in orgs:
            ...
            price  = estimate_monthly_price(org["plan_tier"], org["max_users"])
            ...
            <div class='org-row'>
                ...
                <div style='margin-top:10px;font-size:13px;color:#a0a0c0;'>
                    ...
                    · <strong style='color:#1D9E75;'>{price:,}€/mois</strong>
                </div>
            ...
            with col2:
                with st.popover("Modifier plan"):
                    ...
                    new_max = st.number_input(
                        "Licences", min_value=PLAN_TIERS[new_tier]["min_users"],
                        max_value=PLAN_TIERS[new_tier]["max_users"],
                        value=org["max_users"], key=f"max_{org['id']}",
                    )
                    if st.button("Mettre a jour", key=f"upd_{org['id']}"):
                        update_organization_plan(org["id"], new_tier, new_max)
                        st.rerun()
