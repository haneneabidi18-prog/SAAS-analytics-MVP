...
with st.sidebar:
    ...
    MODEL_OPTIONS = {
        "Standard (recommande)": "standard",
        "Rapide":                "rapide",
    }
    model_label_display = st.selectbox("Modele IA", list(MODEL_OPTIONS.keys()))
    model_choice = MODEL_OPTIONS[model_label_display]
...
if st.session_state["messages"] and st.session_state["messages"][-1]["role"] == "user":
    with st.chat_message("assistant", avatar="🤖"):
        with st.spinner("Analyse en cours..."):
            if not get_provider():
                st.error("⚠️ Aucune cle IA configuree. Contactez l'administrateur de la plateforme.")
                st.stop()
            ...
            try:
                for chunk in stream_response(st.session_state["messages"], system, model_choice):
                    full_response += chunk
