...
from utils.org import list_org_members, add_team_member, remove_team_member, \
                       reset_member_password, PLAN_TIERS, get_org_monthly_price
...
used    = len(members)
max_u   = org["max_users"]
monthly = get_org_monthly_price(org)

# ── Header ────────────────────────────────────────────────────────────────────
st.markdown(f"## 👥 Gestion d'equipe — {org['name']}")
_price_suffix = "(tarif negocie)" if tier.get("custom_pricing") else f"· {tier['price_per_user']}€/utilisateur/mois"
st.markdown(f"""
...
c3.metric("Tarif mensuel estime", f"{monthly:,.0f} €")
